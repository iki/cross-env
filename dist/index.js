...
function splitEnvSetterMatch(match) {
  // eslint-disable-line complexity
  var name = match[1];
  var isDefault = match[2];
  var value = isDefault && process.env[name] || match[4] || match[5] || match[6] || '';
  return { name, value };
}
...
function parseCommand(args) {
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (match) {
      var _splitEnvSetterMatch = splitEnvSetterMatch(match),
          name = _splitEnvSetterMatch.name,
          value = _splitEnvSetterMatch.value;
